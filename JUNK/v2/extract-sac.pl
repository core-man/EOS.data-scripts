...
@ARGV == 12 or die "perl $0 ev_dir start end origin evla evlo evdp ref_flag EOSsta EOSAR cal_time\n";
my ($ev_dir,$start,$end,$origin,$evla,$evlo,$evdp,$ref_flag,$cal_time,$EOSsta,$EOSAR,$band) = @ARGV;
...
#my ($yday_start,$year_start,$mon_start,$day_start,$hour_start) = split " ", &find_time($origin, $start);
    #print STDERR "$origin\n$start_time\n";
...
#my ($yday_end,$year_end,$mon_end,$day_end,$hour_end) = split " ", &find_time($origin, $end);
    #print STDERR "$origin\n$end_time\n";
...
# reference is origin time
    ...
    print STDERR "${year_start} ${mon_start} ${day_start} ${hour_start}#\n";
...
=pod
            ($yday_end,$year_end,$mon_end,$day_end,$hour_end) =
                                split " ", &find_time($origin, $end_new);
...
=cut
    ...
    my @mseeds;
    for (my $j = 0; $j < @hours; $j++) {
        my $data_dir = "$EOSAR/$years[$j]/$net/$sta"; # data directory
...
##############################
# find the day of year, year, month, day, hour, hour when origin is plus duration
sub find_time {
    ...
    print STDERR "$yday $year $mon $day $hour##\n";
